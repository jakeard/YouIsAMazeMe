...
class LevelSelector(arcade.View):
    ...
    def on_draw(self):
        ...
        arcade.draw_text('Levels', int(constants.SCREEN_WIDTH / 2 * constants.WIDTH_RATIO), int(550 * constants.HEIGHT_RATIO), arcade.color.RUST, font_size=35, anchor_x='center')
        ...
        while level != self.num_files:
            for i in range(int(200 * constants.WIDTH_RATIO), int(600 * constants.WIDTH_RATIO) + 1, int(200 * constants.WIDTH_RATIO)):
                ...
                if level % 4 == 0:
                    y -= 150
                if level < 10:
                    arcade.draw_text(f'0{level}', i, y, arcade.color.RUST, font_size=35, anchor_x='center')
                else:
                    arcade.draw_text(f'{level}', i, y, arcade.color.RUST, font_size=35, anchor_x='center')
