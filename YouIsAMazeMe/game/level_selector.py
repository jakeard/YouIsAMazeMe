...
class LevelSelector(arcade.View):
    def __init__(self):
        super().__init__()
        self.press_ranges = []
        self.num_files = len([f for f in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../', 'levels')) if f != 'todo.txt'])

    def on_draw(self):
        arcade.start_render()
        arcade.set_background_color(arcade.color.KHAKI)
        arcade.draw_text('Levels', int(constants.SCREEN_WIDTH / 2 + 1 * constants.WIDTH_RATIO), int(550 * constants.HEIGHT_RATIO), arcade.color.RUST, font_size=35, anchor_x='center')
        arcade.draw_text('Back', int(60 * constants.WIDTH_RATIO), int(25 * constants.HEIGHT_RATIO), arcade.color.RUST, font_size=25, anchor_x='center')
        level = 0
        ...
        while level != self.num_files:
            for i in range(int(200 * constants.WIDTH_RATIO), int(600 * constants.WIDTH_RATIO) + 1, int(200 * constants.WIDTH_RATIO)):
                ...
                if level < 10:
                    arcade.draw_text(f'0{level}', i, int(y), arcade.color.RUST, font_size=35, anchor_x='center')
                else:
                    arcade.draw_text(f'{level}', i, int(y), arcade.color.RUST, font_size=35, anchor_x='center')
